# Mobile port note: adapted from services/api/src/planner/planner_models.py.
# The only change: resolve_missing_timezone no longer auto-detects a
# location's IANA timezone from lat/lon via `timezonefinder` (that package
# bundles a large embedded geographic dataset not worth shipping on-device
# for this). Callers must supply `timezone` explicitly - which our app does
# for every location it defines. Everything else, including all the actual
# astronomy (Observer construction, horizon-mask blocking math), is
# untouched.
