    ...
    class DeprecateWrapper:
        def __getattr__(self, attr):
            if attr in deprecated:
                warnmsg = (
                    "`MAGIC_TIME` will be deprecated in future versions "
...
sys.modules[__name__] = deprecation_wrap_module(
    sys.modules[__name__], deprecated=["MAGIC_TIME"]
)
...
def _process_nans_in_jds(jds):
    ...
    masked_jds = np.ma.masked_array(
        jds.to(u.day).value if hasattr(jds, "unit") else jds.copy()
    )
...
def _generate_24hr_grid(t0, start, end, n_grid_points, for_deriv=False):
    ...
    if for_deriv:
        time_grid = (
            np.concatenate(
                [
                    [start - 1 / (n_grid_points - 1)],
                    np.linspace(start, end, n_grid_points)[1:-1],
                    [end + 1 / (n_grid_points - 1)],
                ]
            )
            * u.day
        )
...
class Observer:
    ...
    def __init__(
        ...
        if location is None and (latitude is not None and longitude is not None):
            ...
            raise TypeError(
                "Observatory location must be specified with "
        ...
        if isinstance(timezone, datetime.tzinfo):
            ...
            raise TypeError(
                "timezone keyword should be a string, or an instance of datetime.tzinfo"
            )
    ...
    def __repr__(self):
        ...
        attributes_strings = []
        for name, value in zip(attr_names, attr_values, strict=False):
            if value is not None:
                # Format location for easy readability
                if name == "location":
                    formatted_loc = [f"{i.value} {i.unit}" for i in value.to_geodetic()]
                    attributes_strings.append(
                        "{} (lon, lat, el)=({})".format(name, ", ".join(formatted_loc))
                    )
                else:
                    if name != "name":
                        value = repr(value)
    ...
    def at_site(cls, site_name, **kwargs):
        ...
        if "location" in kwargs:
            raise ValueError(
                "Location kwarg should not be used if "
    ...
    def _is_broadcastable(self, shp1, shp2):
        ...
        if shp1 == shp2:
            return True
        for a, b in zip(shp1[::-1], shp2[::-1], strict=False):
    ...
    def _preprocess_inputs(self, time, target=None, grid_times_targets=False):
        ...
        if grid_times_targets:
            ...
            raise ValueError(
                "Time and Target arguments cannot be broadcast "
    ...
    def parallactic_angle(
        self, time, target, grid_times_targets=False, kind="mean", model=None
    ):
        """
        ...
        q = (
            np.arctan2(
                np.sin(H),
                (
                    np.tan(self.location.lat.radian) * np.cos(coordinate.dec.radian)
                    - np.sin(coordinate.dec.radian) * np.cos(H)
                ),
            )
            * u.rad
        )
    ...
    def _horiz_cross(self, t, alt, rise_set, horizon=0 * u.degree):
        ...
        if np.any(noncrossing_indices):
            for target_index in set(np.where(noncrossing_indices)[0]):
                warnmsg = (
                    f"Target with index {target_index} does not cross horizon={horizon} "
                    "within 24 hours"
                )
    ...
    @u.quantity_input(horizon=u.deg)
    def _two_point_interp(
        self, jd_before, jd_after, alt_before, alt_after, horizon=0 * u.deg
    ):
        """
    ...
    def _altitude_trig(self, LST, target, grid_times_targets=False):
        ...
        alt = np.arcsin(
            np.sin(self.location.lat.radian) * np.sin(target.dec)
    ...
    def _calc_riseset(
        ...
        if prev_next == "next":
            start = 0
                ...
                target.approx_sidereal_drift.to(u.day).value
                if hasattr(target, "approx_sidereal_drift")
                else 0
            )
        else:
            start = -1 - (
        ...
        return self._two_point_interp(jd1, jd2, al1, al2, horizon=horizon)
    ...
    def _calc_transit(
        ...
        if prev_next == "next":
            times = _generate_24hr_grid(time, 0, 1, n_grid_points, for_deriv=True)
        else:
            times = _generate_24hr_grid(time, -1, 0, n_grid_points, for_deriv=True)
    ...
    def _determine_which_event(self, function, args_dict):
        ...
        if function == self._calc_riseset:
            ...
            raise ValueError(
                f"Function {function} not supported in _determine_which_event."
            )
        ...
        if which == "nearest":
            try:
                ...
                ma = (
                    next_event.utc.jd
                    if not next_event.isscalar
                    else [next_event.utc.jd]
                )
            return Time(_process_nans_in_jds(ma), format="jd", scale="utc")

        raise ValueError('"which" kwarg must be "next", "previous" or "nearest".')
    ...
    def target_rise_time(
        ...
        """
        return self._determine_which_event(
            self._calc_riseset,
            dict(
                time=time,
                target=target,
    ...
    def target_meridian_transit_time(
        self, time, target, which="nearest", grid_times_targets=False, n_grid_points=150
    ):
        """
    ...
    def target_meridian_antitransit_time(
        self, time, target, which="nearest", grid_times_targets=False, n_grid_points=150
    ):
        """
    ...
    @u.quantity_input(horizon=u.deg)
    def sun_rise_time(
        self, time, which="nearest", horizon=0 * u.degree, n_grid_points=150
    ):
        """
        Time of sunrise.
        ...
        """
        return self.target_rise_time(
            time, get_sun(time), which, horizon, n_grid_points=n_grid_points
        )
    ...
    def noon(self, time, which="nearest", n_grid_points=150):
        ...
        `~astropy.time.Time`
            Time at solar noon
        """
        return self.target_meridian_transit_time(
            time, get_sun(time), which, n_grid_points=n_grid_points
        )
    ...
    def midnight(self, time, which="nearest", n_grid_points=150):
        ...
        `~astropy.time.Time`
            Time at solar midnight
        """
        return self.target_meridian_antitransit_time(
            time, get_sun(time), which, n_grid_points=n_grid_points
        )
    ...
    def twilight_evening_astronomical(self, time, which="nearest", n_grid_points=150):
        ...
        return self.sun_set_time(
            time, which, horizon=-18 * u.degree, n_grid_points=n_grid_points
        )
    ...
    def twilight_evening_nautical(self, time, which="nearest", n_grid_points=150):
        ...
        """
        return self.sun_set_time(
            time, which, horizon=-12 * u.degree, n_grid_points=n_grid_points
        )
    ...
    def twilight_evening_civil(self, time, which="nearest", n_grid_points=150):
        ...
        """
        return self.sun_set_time(
            time, which, horizon=-6 * u.degree, n_grid_points=n_grid_points
        )
    ...
    def moon_rise_time(
        self, time, which="nearest", horizon=0 * u.deg, n_grid_points=150
    ):
        """
        ...
        """
        return self.target_rise_time(
            time, MoonFlag, which, horizon, n_grid_points=n_grid_points
        )
    ...
    def local_sidereal_time(self, time, kind="apparent", model=None):
        ...
        return time.sidereal_time(kind, longitude=self.location.lon, model=model)
