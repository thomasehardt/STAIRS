...
_current_year_time_range = Time(  # needed for backward compatibility
    [str(_current_year) + "-01-01", str(_current_year) + "-12-31"]
)
...
def _get_altaz(times, observer, targets, force_zero_pressure=False):
    ...
    if aakey not in observer._altaz_cache:
        try:
            ...
            observer._altaz_cache[aakey] = dict(times=times, altaz=altaz)
        finally:
            if force_zero_pressure:
...
def _get_moon_data(times, observer, force_zero_pressure=False):
    ...
    if aakey not in observer._moon_cache:
        try:
            ...
            observer._moon_cache[aakey] = dict(
                times=times, illum=illumination, altaz=altaz
            )
        finally:
            if force_zero_pressure:
                observer.pressure = observer_old_pressure
    ...
    def __call__(
        ...
        if times is None and time_range is not None:
            times = time_grid_from_range(
                time_range, time_resolution=time_grid_resolution
            )
        ...
        times, targets = observer._preprocess_inputs(
            times, targets, grid_times_targets=False
        )
...
class AirmassConstraint(AltitudeConstraint):
    ...
    def compute_constraint(self, times, observer, targets):
        ...
        if self.boolean_constraint:
            ...
            if self.max is None:
                raise ValueError(
                    "Cannot have a float AirmassConstraint if max is None."
                )
            else:
                mx = self.max
    ...
    def _get_solar_altitudes(self, times, observer, targets):
        ...
        if aakey not in observer._altaz_cache:
            try:
                ...
                altitude = altaz.alt
                # cache the altitude
                observer._altaz_cache[aakey] = dict(times=times, altitude=altitude)
            finally:
                if self.force_pressure_zero:
                    observer.pressure = observer_old_pressure
        else:
...
class GalacticLatitudeConstraint(Constraint):
    ...
    def compute_constraint(self, times, observer, targets):
        ...
        if self.min is None and self.max is not None:
            ...
            raise ValueError(
                "No max and/or min specified in GalacticLatitudeConstraint."
            )
        return mask
...
class SunSeparationConstraint(Constraint):
    ...
    def compute_constraint(self, times, observer, targets):
        ...
        if self.min is None and self.max is not None:
            ...
            mask = (self.min <= solar_separation) & (solar_separation <= self.max)
        else:
            raise ValueError("No max and/or min specified in SunSeparationConstraint.")
        return mask
...
class MoonSeparationConstraint(Constraint):
    ...
    def compute_constraint(self, times, observer, targets):
        moon = get_body(
            "moon", times, location=observer.location, ephemeris=self.ephemeris
        )
        ...
        if self.min is None and self.max is not None:
            ...
            mask = (self.min <= moon_separation) & (moon_separation <= self.max)
        else:
            raise ValueError("No max and/or min specified in MoonSeparationConstraint.")
        return mask
...
class MoonIlluminationConstraint(Constraint):
    ...
    def compute_constraint(self, times, observer, targets):
        ...
        if self.min is None and self.max is not None:
            ...
            mask = (
                (self.min <= illumination) & (illumination <= self.max)
            ) & moon_up_mask
        else:
            raise ValueError("No max and/or min specified in MoonSeparationConstraint.")
...
class LocalTimeConstraint(Constraint):
    ...
    def __init__(self, min=None, max=None):
        ...
        if self.min is None and self.max is None:
            raise ValueError(
                "You must at least supply either a minimum or a maximum time."
            )

        if self.min is not None:
            if not isinstance(self.min, datetime.time):
                raise TypeError(
                    "Time limits must be specified as datetime.time objects."
                )
    ...
    def compute_constraint(self, times, observer, targets):
        ...
        if min_time < max_time:
            try:
                mask = np.array(
                    [min_time <= t.time() <= max_time for t in times.datetime]
                )
        ...
        # If time boundaries straddle midnight:
            ...
            try:
                mask = np.array(
                    [
                        (t.time() >= min_time) or (t.time() <= max_time)
                        for t in times.datetime
                    ]
                )
            except BaseException:
                mask = np.bool_(
                    (times.datetime.time() >= min_time)
                    or (times.datetime.time() <= max_time)
                )
        return mask
    ...
    def __init__(self, min=None, max=None):
        ...
        if self.min is not None:
            if not isinstance(self.min, Time):
                raise TypeError(
                    "Time limits must be specified as astropy.time.Time objects."
                )
...
class PhaseConstraint(Constraint):
    ...
    def __init__(self, periodic_event, min=None, max=None):
        ...
        if (min < 0) or (min > 1) or (max < 0) or (max > 1):
            raise ValueError(
                "The minimum of the PhaseConstraint must be within the interval [0, 1)."
            )
...
def is_event_observable(
    constraints, observer, target, times=None, times_ingress_egress=None
):
    """
    ...
    if times is not None:
        applied_constraints = [
            constraint(observer, target, times=times, grid_times_targets=True)
            for constraint in constraints
        ]
        constraint_arr = np.logical_and.reduce(applied_constraints)
        ...
        applied_constraints_ing = [
            constraint(observer, target, times=times_ing, grid_times_targets=True)
            for constraint in constraints
        ]
        applied_constraints_egr = [
            constraint(observer, target, times=times_egr, grid_times_targets=True)
            for constraint in constraints
        ]

        constraint_arr = np.logical_and(
            np.logical_and.reduce(applied_constraints_ing),
            np.logical_and.reduce(applied_constraints_egr),
        )
    return constraint_arr


def months_observable(
    constraints,
    observer,
    targets,
    ...
    if not altitude_constraint_supplied:
        message = (
            "months_observable usually expects an AltitudeConstraint or "
    ...
    # altitude calculations.
    applied_constraints = [
        constraint(observer, targets, times=times, grid_times_targets=True)
        for constraint in constraints
    ]
    constraint_arr = np.logical_and.reduce(applied_constraints)

    months_observable = []
    for target, observable in zip(targets, constraint_arr, strict=False):
...
def observability_table(
    ...
    tab = table.Table(
        names=colnames, data=[target_names, ever_obs, always_obs, frac_obs]
    )
