...
class FixedTarget(Target):
    ...
    def __init__(self, coord, name=None, **kwargs):
        ...
        name : str (optional)
            ...
            as a string
        """
        if not (hasattr(coord, "transform_to") and hasattr(coord, "represent_as")):
    ...
    def _from_name_mock(cls, query_name, name=None):
        ...
        if query_name.lower() in stars:
            return cls(coord=SkyCoord(**stars[query_name.lower()]), name=query_name)
        else:
            raise ValueError(
                f"Target named {query_name} not in mocked FixedTarget method"
            )
...
def get_skycoord(targets):
    ...
    targets_is_unitsphericalrep = [
        x.data.__class__ is UnitSphericalRepresentation for x in coords
    ]
    ...
    if convert_to_icrs:
        ...
        try:
            ...
            lon_name, lat_name = [
                mapping.framename
                for mapping in coords[0].frame_specific_representation_info[
            ...
            lon_name, lat_name = [
                mapping.framename
                for mapping in coords[0].frame_specific_representation_info["spherical"]
            ]
    ...
    if all(targets_is_unitsphericalrep):
        ...
        """
        distances = [
            distance if distance != 1 else 100 * u.kpc for distance in distances
        ]
        return SkyCoord(longitudes, latitudes, distances, frame=frame)
