...
class ObservingBlock:
    ...
    def __init__(
        self, target, duration, priority, configuration={}, constraints=None, name=None
    ):
        """
        Parameters
        ----------
    ...
    def __repr__(self):
        ...
        if self.start_time is None or self.end_time is None:
            return orig_repr.replace(
                "object at", f"({self.target.name}, unscheduled) at"
            )
        else:
        ...
        for i, block in enumerate(self.blocks):
            ...
            if block.constraints:
                for constraint in block.constraints:
                    applied_score = constraint(self.observer, block.target, times=times)
                    score_array[i] *= applied_score
        for constraint in self.global_constraints:
            score_array *= constraint(
                self.observer, self.targets, times, grid_times_targets=True
            )
        return score_array

    @classmethod
    def from_start_end(
        cls, blocks, observer, start_time, end_time, global_constraints=[]
    ):
        """
...
class TransitionBlock:
    ...
    def __repr__(self):
        orig_repr = object.__repr__(self)
        comp_info = ", ".join([f"{c}: {t}" for c, t in self.components.items()])
        if self.start_time is None or self.end_time is None:
...
class Schedule:
    ...
    def __repr__(self):
        return (
            "Schedule containing "
            + str(len(self.observing_blocks))
            + " observing blocks between "
            + str(self.slots[0].start.iso)
            + " and "
            + str(self.slots[-1].end.iso)
        )

    @property
    def observing_blocks(self):
        return [
            slot.block for slot in self.slots if isinstance(slot.block, ObservingBlock)
        ]
    ...
    def to_table(self, show_transitions=True, show_unused=False):
        ...
        for slot in self.slots:
            if hasattr(slot.block, "target"):
                ...
                config.append("")
        return Table(
            [target_names, start_times, end_times, durations, ra, dec, config],
            names=(
                "target",
                "start time (UTC)",
                "end time (UTC)",
    ...
    def insert_slot(self, start_time, block):
        ...
        for j, slot in enumerate(self.slots):
            if (
                slot.start < start_time or abs(slot.start - start_time) < 1 * u.second
        ...
        if abs(self.slots[slot_index].duration - block.duration) < 1 * u.second:
            ...
            abs(self.slots[slot_index].end - start_time - block.duration) < 1 * u.second
        ):
...
class Scheduler:
    ...
    def __init__(
        ...
        if not callable(self.transitioner):
    ...
    def from_timespan(cls, center_time, duration, **kwargs):
        ...
        """
        start_time = center_time - duration / 2.0
        end_time = center_time + duration / 2.0
...
class SequentialScheduler(Scheduler):
    ...
    def _make_schedule(self, blocks):
        ...
        if len(pre_filled) == 0:
            ...
            filled_times = Time(
                [a - 1 * u.hour, a - 1 * u.hour, a - 1 * u.minute, a - 1 * u.minute]
            )
        ...
        while (len(blocks) > 0) and (current_time < self.schedule.end_time):
            ...
            for b in blocks:
                ...
                if len(self.schedule.observing_blocks) > 0:
                    trans = self.transitioner(
                        self.schedule.observing_blocks[-1],
                        b,
                        current_time,
                        self.observer,
                    )
                ...
                if any(
                    (current_time < filled_times) & (filled_times < times[2])
                ) or any(abs(pre_filled.T[0] - current_time) < 1 * u.second):
            ...
            if block_constraint_results[bestblock_idx] == 0.0:
...
class PriorityScheduler(Scheduler):
    ...
    def _make_schedule(self, blocks):
        ...
        scorer = Scorer(
            blocks, self.observer, self.schedule, global_constraints=self.constraints
        )
            ...
            if np.all(constraint_scores == 0) or np.all(~good):
                ...
                for idx in np.argsort(-sum_scores, kind="mergesort"):
                    ...
                    try:
                        ...
                        _is_scheduled = self.attempt_insert_block(
                            b, new_start_time, start_time_idx
                        )
                        if _is_scheduled:
                            break
                    except IndexError:
    ...
    def attempt_insert_block(self, b, new_start_time, start_time_idx):
        ...
        duration_indices = int(np.floor(float(b.duration / self.time_resolution)))
        ...
        slot_index = [
            q
            for q, slot in enumerate(self.schedule.slots)
            if slot.start < new_start_time + 1 * u.second < slot.end
        ][0]
        ...
        if slots_before:
            if isinstance(self.schedule.slots[slot_index - 1].block, ObservingBlock):
                ...
                tb_before = self.transitioner(
                    self.schedule.slots[slot_index - 1].block,
                    b,
                    self.schedule.slots[slot_index - 1].end,
                    self.observer,
                )
            elif isinstance(self.schedule.slots[slot_index - 1].block, TransitionBlock):
                tb_before = self.transitioner(
                    self.schedule.slots[slot_index - 2].block,
                    b,
                    self.schedule.slots[slot_index - 2].end,
                    self.observer,
                )
        ...
        if slots_after:
            ...
            if isinstance(
                ...
                tb_after = self.transitioner(
                    b,
                    self.schedule.slots[slot_index + slot_offset].block,
                    new_start_time + b.duration,
                    self.observer,
                )
        ...
        try:
            ...
            # this shouldn't ever happen
            print(
                f"Failed to insert {b.target.name} (dur: {b.duration}) into schedule.\n{new_start_time.iso}\n{str(error)}"
            )
            return False
...
class Transitioner:
    ...
    def __call__(self, oldblock, newblock, start_time, observer):
        ...
        components = {}
        if (
            self.slew_rate is not None
            and (oldblock is not None)
            and (newblock is not None)
        ):
