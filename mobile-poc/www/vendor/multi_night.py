# Mobile port note: adapted from services/api/src/planner/multi_night.py.
# All actual scoring/astronomy math (calculate_night_score's moon-quality
# and weather-multiplier convolution, generate_forecast's night-stepping
# loop) is untouched. What's removed:
#   - EphemerisManager's cached-moon-quality lookup (a server-side
#     precomputed-parquet cache) - always falls through to computing
#     directly, exactly like the original code already does on a cache miss.
#   - FileCache's 1-hour result cache and GeoCacheService's cache-key
#     helper - no disk cache on-device; every call just computes fresh.
#   - WeatherService's type import (still accepted as a parameter and
#     honored if truthy, just typed as Any instead of importing the real
#     class, which itself calls a live Open-Meteo API STAIRS hasn't wired
#     up on-device yet).
import logging
...
from scoring import calculate_bortle_multiplier, calculate_weather_score_vectorized
from visibility import get_astronomical_night
