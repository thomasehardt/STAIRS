...
class TestScoring(TestCase):
    def test_get_target_size_fov(self):
        ...
        mock_target_none_size = MockTarget(
            angular_size=None, target_type="Emission Nebula"
        )
        ...
        mock_target_empty_size = MockTarget(
            angular_size=np.array([]), target_type="Star"
        )
        self.assertEqual(scoring.get_target_size_fov(mock_target_empty_size), 0.01)

        # Test with Pandas Series
        mock_series = pd.Series(
            {"angular_size": 15.0, "target_type": "Globular Cluster"}
        )
        self.assertEqual(scoring.get_target_size_fov(mock_series), 15.0)

        mock_series_none = pd.Series({"target_type": "Galaxy"})
    ...
    def test_calculate_altitude_score(self):
        ...
        np.testing.assert_allclose(
            scoring.calculate_altitude_score(max_altitudes, min_alt),
            expected_scores,
            atol=1e-4,
        )
        ...
        np.testing.assert_allclose(
            scoring.calculate_altitude_score(max_altitudes_low, min_alt),
            expected_scores_low,
            atol=1e-4,
        )
        ...
        np.testing.assert_allclose(
            scoring.calculate_altitude_score(max_altitudes_high_min, min_alt_high),
            expected_scores_high_min,
            atol=1e-4,
        )

        np.testing.assert_array_equal(
            scoring.calculate_altitude_score([]), np.array([])
        )

        max_altitudes_clip = np.array([-10.0, 95.0])
        expected_scores_clip = np.array([0.0, 100.0])
        np.testing.assert_allclose(
            scoring.calculate_altitude_score(max_altitudes_clip, min_alt),
            expected_scores_clip,
            atol=1e-4,
        )
    ...
    def test_calculate_sb_score(self):
        ...
        np.testing.assert_allclose(
            scoring.calculate_sb_score(sbs), expected_scores, atol=1e-4
        )

        sbs_clip = np.array([10.0, 30.0])
        expected_scores_clip = np.array([100.0, 0.0])
        np.testing.assert_allclose(
            scoring.calculate_sb_score(sbs_clip), expected_scores_clip, atol=1e-4
        )
    ...
    def test_calculate_weather_score_vectorized(self):
        ...
        seeing = np.array([1.5])
        np.testing.assert_allclose(
            scoring.calculate_weather_score_vectorized(clouds, humidity, seeing),
            np.array([0.95]),
            atol=1e-4,
        )
        ...
        np.testing.assert_array_equal(
            scoring.calculate_weather_score_vectorized([], [], []), np.array([])
        )
        np.testing.assert_array_equal(
            scoring.calculate_weather_score_vectorized([], []), np.array([])
        )
    ...
    def test_calculate_weather_score(self):
        ...
        self.assertAlmostEqual(
            scoring.calculate_weather_score(weather_data_missing), 1.0
        )
    ...
    def test_calculate_sqs_vectorized(self):
        ...
        np.testing.assert_allclose(
            scoring.calculate_sqs_vectorized(alts, azs, 1.0, 1.0), expected, atol=1e-4
        )
        ...
        np.testing.assert_allclose(
            scoring.calculate_sqs_vectorized(alts_zenith, azs_meridian, 1.0, 1.0),
            expected_zenith_meridian,
            atol=1e-4,
        )
        ...
        np.testing.assert_allclose(
            scoring.calculate_sqs_vectorized(alts_combo, azs_combo, 0.5, 0.9),
            np.array([32.4]),
            atol=1e-4,
        )
        ...
        np.testing.assert_allclose(
            scoring.calculate_sqs_vectorized(alts_clip, azs_clip, 1.0, 1.0),
            expected_clip,
            atol=1e-4,
        )

        np.testing.assert_array_equal(
            scoring.calculate_sqs_vectorized([], [], 1.0, 1.0), np.array([])
        )

    def test_calculate_sqs(self):
        self.assertAlmostEqual(
            scoring.calculate_sqs(
                current_altaz=None, weather_data=None, moon_multiplier=1.0
            ),
            100.0,
        )
        ...
        self.assertAlmostEqual(
            scoring.calculate_sqs(
                current_altaz=mock_altaz_high, weather_data=None, moon_multiplier=1.0
            ),
            100.0,
        )
        ...
        self.assertAlmostEqual(
            scoring.calculate_sqs(
                current_altaz=MockAltAz(alt_deg=60.0, az_deg=100.0),
                weather_data=weather_data_good,
                moon_multiplier=0.5,
            ),
            50.0,
        )
    ...
    def test_calculate_oss_vectorized(self):
        ...
        oss_rel, aqs_abs = scoring.calculate_oss_vectorized(
            targets_df, profile, min_target_altitude
        )
        ...
        oss_rel_bortle7, aqs_abs_bortle7 = scoring.calculate_oss_vectorized(
            targets_df, profile, min_target_altitude, bortle_scale=bortle_scale_7
        )
        ...
        oss_rel_missing, aqs_abs_missing = scoring.calculate_oss_vectorized(
            targets_df_size_attr, profile, min_target_altitude
        )
        ...
        empty_df = pd.DataFrame(
            columns=["magnitude", "peak_alt", "size_maj", "size_min", "target_type"]
        )
        oss_rel_empty, aqs_abs_empty = scoring.calculate_oss_vectorized(
            empty_df, profile, min_target_altitude
        )
    ...
    def test_calculate_oss(self):
        ...
        oss_rel_dict, aqs_abs_dict = scoring.calculate_oss(
            target_dict,
            profile,
            max_altitude,
            min_target_altitude,
        ...
        oss_rel_model, aqs_abs_model = scoring.calculate_oss(
            target_model,
            profile,
            max_altitude,
            min_target_altitude,
        ...
        oss_rel_angular, aqs_abs_angular = scoring.calculate_oss(
            target_dict_only_angular,
            profile,
            max_altitude,
            min_target_altitude,
        ...
        oss_rel_fallback, aqs_abs_fallback = scoring.calculate_oss(
            target_dict_fallback,
            profile,
            max_altitude,
            min_target_altitude,
        ...
        oss_rel_todict, aqs_abs_todict = scoring.calculate_oss(
            target_to_dict,
            profile,
            max_altitude,
            min_target_altitude,
