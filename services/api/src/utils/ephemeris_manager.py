...
class EphemerisManager:
    ...
    async def warm_up_cache(self, days: int = 30) -> None:
        ...
        try:
            ...
            targets_df = db.execute(
                "SELECT identifier, ra_deg, dec_deg FROM targets"
            ).df()
            if targets_df.empty:
            ...
            for _, loc_row in locations_df.iterrows():
                ...
                for day_idx in range(days):
                    ...
                    p_path = self.get_peak_alt_path(
                        loc.latitude, loc.longitude, check_date
                    )
                    m_path = self.get_moon_qual_path(
                        loc.latitude, loc.longitude, check_date
                    )

                    if not p_path.exists() or not m_path.exists():
                        # We need the actual night window to be sure
                        night = get_astronomical_night(
                            observer,
                            Time(
                                datetime.combine(
                                    check_date, datetime.min.time(), tzinfo=UTC
                                )
                            ),
                        )
                        if night:
                ...
                logger.info(
                    "Warming up ephemeris cache for {len(missing_nights)} "
                    "nights at {loc.name}"
                )
                work_performed = True
                ...
                for i, (n_start, n_end) in enumerate(missing_nights):
                    ...
                    if num_slots > 0:
                        times = n_start + np.arange(num_slots) * TimeDelta(
                            15 * u.minute
                        )
                ...
                if all_times:
                    flat_times = Time(
                        np.concatenate([t.jd for t in all_times]), format="jd"
                    )
                    ...
                    # Illumination per night (sampled at
                    # midnight for simplicity in batch)
                    night_mids = Time(
                        [(n[0].jd + n[1].jd) / 2 for n in missing_nights], format="jd"
                    )
                    night_ills = np.array(
                        [observer.moon_illumination(t) for t in night_mids]
                    )

                    flat_ills = night_ills[night_indices]
                    flat_moon_quals = 1.0 - (
                        flat_ills * 0.5 * (np.maximum(0, moon_alts) / 90.0)
                    )
                    ...
                    for i, (n_start, _) in enumerate(missing_nights):
                        ...
                        mq_df = pd.DataFrame(
                            {
                                "timestamp": [
                                    t.to_datetime(timezone=UTC) for t in all_times[i]
                                ],
                                "moon_quality": flat_moon_quals[
                                    cursor : cursor + n_slots
                                ],
                            }
                        )
                        self.get_moon_qual_path(
                            loc.latitude,
                            loc.longitude,
                            n_start.to_datetime(timezone=UTC).date(),
                ...
                # We sample 5 points per night. For N nights,
                # we calculate 5N samples for all targets.
                sample_times_list = []
                for n_start, n_end in missing_nights:
                    duration = n_end - n_start
                    sample_times_list.append(n_start + np.linspace(0, 1, 5) * duration)

                flat_sample_times = Time(
                    np.concatenate([t.jd for t in sample_times_list]), format="jd"
                )

                # result shape: (num_targets, num_total_samples)
                altaz_frame = AltAz(
                    obstime=flat_sample_times[np.newaxis, :],
                    location=observer.location,
                )
                # Note: this can be memory intensive for huge target lists, but
                # STAIRS targets are usually < 20k
                ...
                for i, (n_start, _) in enumerate(missing_nights):
                    ...
                    alts_df = pd.DataFrame(
                        {"identifier": target_ids, "peak_alt": peak_alts}
                    )
                    self.get_peak_alt_path(
                        loc.latitude,
                        loc.longitude,
                        n_start.to_datetime(timezone=UTC).date(),
                    ).parent.mkdir(parents=True, exist_ok=True)
