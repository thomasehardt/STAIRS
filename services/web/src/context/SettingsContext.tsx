...
interface SettingsContextType {
  ...
  // actions
  updatePlanning: (updates: Partial<components['schemas']['PlanningSettings']>) => void;
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const activeLocation = config?.locations.find(loc => loc.default) || config?.locations[0];
  ...
  const updatePlanning = (updates: Partial<components['schemas']['PlanningSettings']>) => {
    if (!config) return;
  ...
  const addLocation = (newLocation: components['schemas']['LocationConfig']) => {
    if (!config) return;
  ...
  const deleteLocation = (locationName: string) => {
    ...
    const newLocations = config.locations.filter(loc => loc.name !== locationName);
    updateMutation.mutate({
      locations: newLocations
    });
  };
