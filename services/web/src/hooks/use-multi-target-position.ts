...
export function useMultiTargetPosition(ids: string[] | undefined, nightStart: Date | null, nightEnd: Date | null) {
  const { activeLocation } = useSettings();

  const posQueries = (ids || []).map((id) => ({
    queryKey: ['target-position', id, activeLocation?.name, nightStart?.toISOString()],
    queryFn: async () => {
      if (!activeLocation || !nightStart || !nightEnd) return null;

      const durationHours = (nightEnd.getTime() - nightStart.getTime()) / (1000 * 60 * 60);
      ...
      const { data } = await api.get<TargetPositionSeries>(`/targets/${id}/position?${params.toString()}`);
      return data;
    },
