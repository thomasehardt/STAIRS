...
export function useExportCsv() {
  return useMutation({
    mutationFn: async (request: PlanRequest) => {
      ...
      const filename =
        headers["content-disposition"]?.split("filename=")[1] || "plan.csv";
...
export function useExportSkylist() {
  return useMutation({
    mutationFn: async (request: PlanRequest) => {
      ...
      const filename =
        headers["content-disposition"]?.split("filename=")[1] || "plan.skylist";
      const url = window.URL.createObjectURL(new Blob([data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", filename.replace(/"/g, ""));
...
export function useForecast(params: {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get<components["schemas"]["ForecastResponse"]>(
