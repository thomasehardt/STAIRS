...
export function useTargets(options: TargetFilterOptions = {}) {
  const { search, catalog, type, maxMagnitude, constellation, limit = 50 } = options;

  return useQuery({
    queryKey: ['targets', search, catalog, type, maxMagnitude, constellation, limit],
    queryFn: async () => {
      ...
      if (constellation && constellation !== 'All Constellations') params.append('constellation', constellation);
      if (maxMagnitude !== undefined && maxMagnitude !== 20) params.append('max_magnitude', maxMagnitude.toString());
      params.append('limit', limit.toString());

      const { data } = await api.get<TargetSearchResponse>(`/targets/search?${params.toString()}`);
      return data;
    },
