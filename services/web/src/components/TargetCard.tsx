  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center px-1">
        <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Season</span>
        <span className="text-[10px] font-black uppercase text-primary tracking-widest">{season || 'All Year'}</span>
      </div>
      <div className="flex gap-0.5 h-1.5 w-full bg-secondary/30 rounded-full overflow-hidden">
        {months.map((_, i) => (
          <div
            key={i}
            className={`flex-1 transition-colors ${activeMonths.includes(i) ? 'bg-primary' : 'bg-transparent'}`}
            title={months[i]}
          />
        ))}
      </div>
      <div className="flex justify-between px-0.5">
        {['Jan', 'Dec'].map(m => <span key={m} className="text-[9px] font-bold text-muted-foreground/40 uppercase">{m}</span>)}
      </div>
    </div>
  );
}
...
function FovPreview({ targetSize, telescopeName }: { targetSize: number[] | null, telescopeName: string }) {
  const { data: profilesData } = useProfiles();

  const fov = useMemo(() => {
    if (!profilesData?.profiles || !targetSize || targetSize.length === 0) return null;
    ...
    const calcFov = (sensorPx: number, pixelPitchUm: number, focalLengthMm: number) => {
      const sensorSizeMm = (sensorPx * pixelPitchUm) / 1000;
      return 2 * Math.atan(sensorSizeMm / (2 * focalLengthMm)) * (180 / Math.PI) * 60;
    };

    const fovX = calcFov(profile.sensor_x, profile.pixel_pitch_um, profile.focal_length_mm);
    const fovY = calcFov(profile.sensor_y, profile.pixel_pitch_um, profile.focal_length_mm);
  ...
  return (
    <div className="space-y-1.5">
        <div className="flex justify-between items-center px-1">
            <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">FOV Scale</span>
            <span className="text-[10px] font-black uppercase text-primary tracking-widest">{targetSize?.[0]} arcmin</span>
        </div>
...
export function TargetCard({
  ...
  const transitInfo = useMemo(() => {
    ...
    return {
        ...
        time: new Date(peak.time).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })
    };
  }, [posData]);

  return (
    <div
        className={`p-5 transition-all border rounded-[2.5rem] bg-card hover:border-primary/40 hover:shadow-xl group relative flex flex-col min-h-[400px] ${isPinned ? 'border-primary shadow-md ring-1 ring-primary/20' : 'border-border'} ${onTogglePin ? 'cursor-pointer' : ''}`}
    ...
    >
        ...
        <div className="flex items-start justify-between gap-2">
            ...
            {onTogglePin && (
                <Button
                    variant="ghost"
                    size="icon"
                    className={`w-9 h-9 rounded-full shrink-0 transition-all ${isPinned ? 'bg-primary text-white hover:bg-primary/90 shadow-lg' : 'bg-secondary/50 text-muted-foreground hover:bg-secondary'}`}
                    onClick={(e) => {
                        e.stopPropagation();
                        onTogglePin(id);
                    }}
            ...
            {target.sqs_score !== undefined && (
                <div className="flex items-center gap-3">
                    <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-black text-primary leading-none">{Math.round(target.sqs_score)}</span>
                        <span className="text-[10px] font-black text-primary/50 uppercase tracking-tighter">REL</span>
                    </div>
                    <div className="w-px h-4 bg-border" />
                    <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-black text-primary leading-none">{Math.round(target.aqs_score || target.oss_score)}</span>
                        <span className="text-[10px] font-black text-primary/50 uppercase tracking-tighter">ABS</span>
                    </div>
                </div>
            )}
        </div>
      ...
      <div className="flex-1 flex flex-col space-y-6">
        ...
        <div className="space-y-4">
            <FovPreview targetSize={target.angular_size} telescopeName={activeTelescope} />
            <SeasonalBar season={target.season} />
        </div>

        {/* Technical Specs */}
        <div className="grid grid-cols-1 gap-2">
            <DetailItem label="Const" value={target.constellation} icon={MapPin} />
            <DetailItem label="Mag" value={target.magnitude ?? 'N/A'} icon={Activity} />
        </div>
            ...
            {variant === 'catalog' ? (
                transitInfo ? (
                    <div className="bg-secondary/30 p-4 rounded-[1.5rem] border border-border/50">
                        <p className="text-[10px] font-black uppercase text-muted-foreground mb-1 leading-none">Tonight's Peak Transit</p>
                        <p className="text-xl font-black text-primary leading-none">{transitInfo.alt}° @ {transitInfo.time}</p>
                    </div>
                ) : null
            ) : (
                <div className="h-32">
                    {posLoading ? (
                        <div className="h-full w-full animate-pulse bg-secondary/20 rounded-2xl flex items-center justify-center">
                            <span className="text-[10px] text-muted-foreground uppercase tracking-widest">Calculating Orbit...</span>
                        </div>
                    ) : posData ? (
    ...
    return (
      <div className="flex items-center gap-2 px-2 py-1.5 rounded-xl bg-background/50 border border-border/50">
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-[10px] uppercase font-black text-muted-foreground leading-none tracking-widest">{label}</span>
