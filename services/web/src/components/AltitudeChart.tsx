...
export function AltitudeChart({ data }: { data: AltitudePoint[] }) {
  ...
  const processedData = data.map((p) => {
    ...
    const offsetHours = (date.getTime() - firstDate.getTime()) / (1000 * 60 * 60);
  ...
  const getChartData = (isMockupView: boolean) => {
    const optimalColor = isMockupView ? COLOR_MOCKUP_OPTIMAL : COLOR_DASH_OPTIMAL;
    const restrictedColor = isMockupView ? COLOR_MOCKUP_RESTRICTED : COLOR_DASH_RESTRICTED;
      ...
      datasets: [
        {
          ...
          segment: {
            borderColor: (ctx: any) => {
              ...
              const avg = (y0 + y1) / 2;
              if (avg >= minAltitude - 0.1 && avg <= maxAltitude + 0.1) return optimalColor;
      ...
      scales: {
        x: {
          ...
          title: {
            ...
            text: 'Time',
            font: { family: isMockupView ? 'Inter, sans-serif' : 'Inter, sans-serif', size: 10, weight: 500 },
            color: textColor
          },
          ticks: {
            ...
            font: { family: isMockupView ? 'Inter, sans-serif' : 'Inter, sans-serif', size: isMockupView ? 10 : 8 },
            color: textColor
          },
          grid: { display: false }
        },
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: (item) => ` Altitude: ${item.parsed.y?.toFixed(1) ?? '0.0'}°`
          }
        },
        annotation: {
          annotations: {
            shading: {
              ...
              yMin: minAltitude,
              yMax: maxAltitude,
              backgroundColor: isMockupView ? 'rgba(31, 119, 180, 0.1)' : `${colors.primary}0D`,
              borderWidth: 0,
              z: -1
            },
            lineMin: {
              ...
              yMin: minAltitude,
              yMax: minAltitude,
              borderColor: isMockupView ? COLOR_MOCKUP_REFERENCE : colors.border,
              ...
              label: {
                ...
                font: { family: isMockupView ? 'Inter, sans-serif' : 'Inter, sans-serif', size: 9, weight: 600 }
              }
            },
            lineMax: {
              type: 'line',
            ...
            linePeak: {
              ...
              label: {
                ...
                font: { family: isMockupView ? 'Inter, sans-serif' : 'Inter, sans-serif', size: 9 }
              }
            }
          }
        }
    ...
    <div ref={containerRef}>
      ...
      >
        <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/5 transition-colors z-10 flex items-center justify-center opacity-0 group-hover:opacity-100">
          <span className="bg-card px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm border border-border text-primary">Open Reference View</span>
        </div>
          ...
          >
            ...
            <div className="bg-white p-6 border-b border-gray-100 flex justify-between items-start">
              <div className="space-y-1">
                <h1 className="text-xl font-bold text-gray-800">Sky Object Altitude</h1>
                <p className="text-gray-500 text-xs italic">Path of object from sunrise to sunset (Chart.js Implementation)</p>
              </div>
              <button
                onClick={() => setShowMockup(false)}
            ...
            <div className="flex-1 overflow-auto p-4 md:p-8">
              <div className="bg-white p-6 md:p-12 rounded-xl shadow-lg flex flex-col min-h-[500px]">
                ...
                <div className="flex justify-between items-center text-[10px] text-gray-400 border-t border-gray-100 pt-6 mt-6">
                  ...
                  <div className="font-medium tracking-tight">STAIRS Instrumentation v3.1</div>
                </div>
              </div>
            </div>
          </div>
