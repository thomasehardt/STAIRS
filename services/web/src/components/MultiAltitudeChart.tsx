...
export function MultiAltitudeChart({ targetDatasets, qualityPoints }: MultiAltitudeChartProps) {
    ...
    if (qualityPoints && qualityPoints.length > 0) {
      datasets.push({
        ...
        data: qualityPoints.map(p => {
          ...
          const offsetHours = (date.getTime() - refTime.getTime()) / (1000 * 60 * 60);
          return {
      ...
      datasets.push({
        ...
        segment: {
          borderColor: (ctx: any) => {
            ...
            const avg = (y0 + y1) / 2;
            if (avg >= minAltitude - 0.1 && avg <= maxAltitude + 0.1) return color;
  ...
  const xValues = chartData.datasets.flatMap(ds => ds.data.map((d: any) => d.x));
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: (item) => {
            ...
            if (label === 'Imaging Quality') return ` Quality: ${Math.round(val)}%`;
