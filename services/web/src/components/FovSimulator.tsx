...
export function FovSimulator({ targetSize, targetName, defaultTelescopeName }: FovSimulatorProps) {
  const { data: profilesData } = useProfiles();
  const [selectedTelescopeName, setSelectedTelescopeName] = useState(defaultTelescopeName);
  const [rotation, setRotation] = useState(0);
  const [zoom, setZoom] = useState(1);

  const selectedTelescope = useMemo(() => {
    return profilesData?.profiles.find(p => p.name === selectedTelescopeName) ||
  ...
  const fov = useMemo(() => {
    if (!selectedTelescope || !targetSize || targetSize.length === 0) return null;
    ...
    const fovX = calcFov(selectedTelescope.sensor_x, selectedTelescope.pixel_pitch_um, selectedTelescope.focal_length_mm);
    const fovY = calcFov(selectedTelescope.sensor_y, selectedTelescope.pixel_pitch_um, selectedTelescope.focal_length_mm);
    ...
    <div className="flex flex-col gap-6">
      <div className="relative aspect-square w-full max-w-[500px] mx-auto bg-slate-950 rounded-[2.5rem] border border-slate-800 shadow-2xl overflow-hidden group">
        ...
        <div className="absolute top-6 left-6 space-y-1">
            <p className="text-[10px] font-black uppercase text-primary tracking-widest">Field of View</p>
            <p className="text-xl font-black text-white">{selectedTelescope.name}</p>
        </div>

        <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
            <div className="bg-black/40 backdrop-blur-md p-3 rounded-2xl border border-white/10 space-y-1">
                <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${fov.fits ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]'}`} />
            ...
            <div className="text-right">
                <p className="text-3xl font-black text-white leading-none">{rotation}&deg;</p>
                <p className="text-[10px] font-black text-primary/60 uppercase tracking-widest">Rotation</p>
            </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-card border border-border rounded-2xl space-y-4">
                <div className="flex items-center justify-between px-1">
                    ...
                    <span className="text-xs font-mono font-bold text-primary">{rotation}&deg;</span>
                </div>
                <input
        ...
        <div className="p-4 bg-card border border-border rounded-2xl space-y-4">
            <div className="flex items-center justify-between px-1">
                ...
                <span className="text-xs font-mono font-bold text-primary">{zoom.toFixed(1)}x</span>
            </div>
