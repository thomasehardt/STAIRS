...
export function PlanGenerator() {
  ...
  useEffect(() => {
    if (state?.initialLocationName && config) {
      const targetLoc = config.locations.find(
        (l) => l.name === state.initialLocationName,
      );
  ...
  const currentLocation =
    ...
    (state?.initialLocationName
      ? {
          name: state.initialLocationName,
          latitude: 0,
          longitude: 0,
          elevation_m: 0,
          bortle_scale: 0,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-lg">
            Create a customized observation schedule for tonight.
          </p>
        </div>
            ...
            <div className="p-6 border rounded-3xl bg-card space-y-4 shadow-sm animate-in slide-in-from-left duration-300">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1 scrollbar-thin">
                {pinnedTargets.map((pt) => (
                  <div
                    key={pt.id}
                    className="flex items-center justify-between p-3 bg-secondary/30 rounded-2xl border border-border/50 group hover:border-primary/30 transition-colors"
                  >
                    <div className="flex flex-col min-w-0 pr-2">
                      ...
                      {pt.common_name && (
                        <span className="text-[10px] text-primary font-bold truncate leading-tight mt-0.5">
                          {pt.common_name}
                        </span>
                      )}
              ...
              <p className="text-[9px] text-muted-foreground italic text-center">
                AI will build the optimal timeline for these targets.
              </p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 flex-1">
                  ...
                  <SummaryCard
                    ...
                    value={
                      generatePlan.data.astronomical_night_start &&
                      generatePlan.data.astronomical_night_end
                        ? `${(
                            (new Date(
                              generatePlan.data.astronomical_night_end,
                            ).getTime() -
                          ...
                          <div className="p-5 border rounded-3xl bg-card hover:border-primary/40 transition-all hover:shadow-md group/card">
                            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                              <div className="space-y-2">
                                ...
                                <div>
                                  <h4 className="text-xl font-black tracking-tight group-hover/card:text-primary transition-colors">
                                    {block.target_id}
                                  </h4>
          ...
          {!discoveryData && !generatePlan.isSuccess && (
            <div className="h-96 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-3xl bg-card/50 text-muted-foreground group">
              ...
              <p className="text-sm max-w-xs text-center mt-2 opacity-60">
                Click "Discover Targets" to find the best objects visible
                tonight, or use the search to add specific ones.
              </p>
            </div>
          )}
