    ...
    return (
        <div className="max-w-4xl space-y-10">
            <div>
                ...
                <p className="text-muted-foreground text-lg">Configure application settings</p>
            </div>
                    ...
                    {isAdding ? (
                        <div className="p-4 border-2 border-primary/30 rounded-2xl bg-card space-y-3">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                ...
                                {/* bortle */}
                                <div className="space-y1">
                                    <label className="text-[10px] font-bold uppercase text-muted-foreground ml-1">Bortle</label>
                                    <input
            ...
            <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {profilesData?.profiles.map((profile) => (
                        <button
                            key={profile.name}
                            onClick={() => updatePlanning({default_telescope: profile.name })}
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-end">
                                <label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Min Altitude</label>
                                <span className="text-2xl font-black text-primary">{config?.planning.min_altitude}&deg;</span>
                            </div>
                            <input
                                type="range" min="0" max="45" step="5"
                                value={config?.planning.min_altitude || 25}
                                onChange={(e) => updatePlanning({ min_altitude: Number(e.target.value) })}
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-end">
                                <label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Max Altitude</label>
                                <span className="text-2xl font-black text-primary">{config?.planning.max_altitude}&deg;</span>
                            </div>
                            <input
                                type="range" min="45" max="90" step="5"
                                value={config?.planning.max_altitude || 75}
                                onChange={(e) => updatePlanning({ max_altitude: Number(e.target.value) })}
                                className="w-full h-2 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary"
                            />
                        </div>
                    </div>
                ...
                <div className="p-8 border rounded-3xl bg-card space-y-8">
                    ...
                    <div className="space-y-4">
                        <label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Temperature</label>
                        ...
                        <p className="text-xs text-muted-foreground italic">
                            This setting is saved to your global configuration and applies to all weather displays.
                        </p>
                    </div>
                </div>
            </section>
