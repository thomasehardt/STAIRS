...
import { useMultiLocationForecast, type MultiForecastResult } from "@/hooks/use-multi-location-forecast";
import { CloudMoon, CalendarDays, MapPin, Clock, ArrowRight, AlertCircle, Info, Moon, Sunrise, Sunset, X } from 'lucide-react';
...
const QualityBadge: React.FC<{ score: number | null | undefined }> = ({ score }) => {
  let color = 'bg-muted text-muted-foreground';
  let text = 'N/A';

  if (score !== null && score !== undefined) {
    text = Math.round(score).toString();
    if (score >= 80) color = 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20';
    else if (score >= 60) color = 'bg-blue-500/10 text-blue-500 border border-blue-500/20';
    else if (score >= 40) color = 'bg-amber-500/10 text-amber-500 border border-amber-500/20';
    else color = 'bg-rose-500/10 text-rose-500 border border-rose-500/20';
  }

  return (
    <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${color}`}>
      {text}
    </span>
  );
};
...
const LocationNightCard: React.FC<{
  ...
  const { data: qualityData, isLoading } = useSkyQuality(loc.name, loc.astronomical_night_start || loc.date);
    ...
    >
      <div className="flex justify-between items-start mb-6">
        <div className="space-y-1 min-w-0 flex-1 pr-2">
          <div className="flex items-center gap-2 mb-1">
            <MapPin className={`w-4 h-4 shrink-0 ${isBest ? 'text-primary' : 'text-muted-foreground'}`} />
            <span className="text-lg font-black uppercase tracking-tight truncate">{loc.name}</span>
            {isBest && (
              <span className="bg-primary text-primary-foreground text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest shrink-0">Best Site</span>
            )}
          </div>
          <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground font-medium">
             <div className="flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" />
                <span>{loc.effective_hours ? `${loc.effective_hours.toFixed(1)}h window` : 'No window'}</span>
             </div>
             <div className="flex items-center gap-1.5">
        ...
        <div className="flex gap-4 items-start shrink-0">
          <div className="flex flex-col items-end gap-1">
            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">Absolute</p>
            <QualityBadge score={loc.absolute_quality} />
          </div>
          <div className="flex flex-col items-end gap-1">
            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">Relative</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="p-4 bg-background/50 rounded-2xl border border-border/50">
          <div className="flex items-center gap-2 mb-2 text-muted-foreground">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">Astro Start</span>
          </div>
          <p className="text-xl font-black font-mono">{formatTime(loc.astronomical_night_start)}</p>
        </div>
        <div className="p-4 bg-background/50 rounded-2xl border border-border/50">
          <div className="flex items-center gap-2 mb-2 text-muted-foreground">
            <Sunset className="w-3.5 h-3.5" />
            <span className="text-[10px] font-black uppercase tracking-widest">Astro End</span>
          </div>
          <p className="text-xl font-black font-mono">{formatTime(loc.astronomical_night_end)}</p>
        </div>
      </div>
      ...
      <div className="mb-6 flex-1 min-h-[140px] relative group/chart">
        <div className="absolute inset-0 flex flex-col">
            <p className="text-[9px] font-black text-primary uppercase tracking-widest mb-2 px-1 flex justify-between">
                ...
                {qualityData?.points && <span className="text-muted-foreground group-hover/chart:text-primary transition-colors opacity-0 group-hover/chart:opacity-100">Live Simulation</span>}
            </p>
            <div className="flex-1 bg-background/30 rounded-2xl border border-border/50 p-4">
                {isLoading ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                        <p className="text-[10px] text-muted-foreground uppercase font-black">No Simulation Available</p>
                    </div>
                )}
            </div>
        </div>
      ...
      {loc.note && (
        <div className="flex items-start gap-3 mb-6 p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed italic">{loc.note}</p>
        </div>
      )}
...
const DayOverview: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-background/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div
    ...
    <div className="group relative">
      ...
      <div className="absolute bottom-full right-0 mb-2 w-64 p-3 bg-card border border-border rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none">
        <div className="space-y-2 text-left">
          <div>
            <p className="text-[10px] font-black uppercase text-primary mb-0.5">Relative Quality</p>
            <p className="text-[10px] text-muted-foreground leading-tight font-medium">
              Measures how good the night is <span className="text-foreground font-bold">for that specific location</span> based on weather, clouds, and moon phase. A 100 means perfect conditions for that site.
            </p>
          </div>
          <div className="pt-2 border-t border-border/50">
            <p className="text-[10px] font-black uppercase text-primary mb-0.5">Absolute Quality</p>
            <p className="text-[10px] text-muted-foreground leading-tight font-medium">
              Adjusts the score based on <span className="text-foreground font-bold">light pollution (Bortle scale)</span>. Allows for a true comparison between sites; a Bortle 8 site will never reach an absolute 100.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-6 bg-card border border-border rounded-3xl shadow-sm mb-8">
      <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            />
            <span className="text-lg font-black font-mono">{days} <span className="text-xs text-muted-foreground uppercase font-sans">Days</span></span>
          </div>
      </div>
      ...
      <div className="flex flex-col gap-1 items-start md:items-end">
          ...
          <div className="flex bg-secondary/50 p-1 rounded-xl mt-2 border border-border/50">
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${qualityType === 'absolute' ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Absolute
            </button>
            <button
              onClick={() => setQualityType('relative')}
              className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${qualityType === 'relative' ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Relative
            </button>
          </div>
...
export const ForecastPage: React.FC = () => {
  ...
  const [qualityType, setQualityType] = useState<'absolute' | 'relative'>('absolute');
  ...
  const filteredLocations = useMemo(() => {
    ...
    return config.locations.filter(l =>
      selectedLocationNames.includes(l.name) || l.name === activeName || l.default
    );
  }, [config?.locations, selectedLocationNames, activeLocation?.name]);

  const results = useMultiLocationForecast(filteredLocations, days) as UseQueryResult<MultiForecastResult>[];
    ...
    const datasets = results
      ...
      .map((r, idx) => {
        ...
        const chartPoints = data.forecastData.days.map(day => ({
          ...
          y: qualityType === 'absolute' ? day.absolute_quality : day.relative_quality,
        }));
    ...
    return {
      ...
      scales: {
        ...
        y: {
          ...
          title: {
            display: true,
            text: qualityType === 'absolute' ? 'Absolute Quality (0-100)' : 'Relative Quality (0-100)',
            font: {
              family: 'JetBrains Mono',
              size: 10,
              weight: 'bold',
  ...
  const daysGrid = useMemo(() => {
    ...
    results.forEach((res) => {
        if (res.status === 'success' && res.data) {
            res.data.forecastData.days.forEach((day) => {
                ...
                const score = qualityType === 'absolute' ? day.absolute_quality : day.relative_quality;
    ...
    const sortedDays = Object.values(allDays)
      ...
      .map(day => {
        ...
        const byScore = [...day.locations].sort((a, b) => b.displayScore - a.displayScore);
        ...
        const displayLocations = [...day.locations].sort((a, b) => {
          const isADefault = a.name === activeName || (config?.locations.find(l => l.name === a.name)?.default);
          const isBDefault = b.name === activeName || (config?.locations.find(l => l.name === b.name)?.default);
    ...
    const firstForecastDate = new Date(filteredForecastDays[0].date + 'T00:00:00');
    const dayOfWeek = firstForecastDate.getDay(); // 0 (Sun) to 6 (Sat)

    const result: (typeof filteredForecastDays[0] | { date: string, isPast: true })[] = [];
  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight uppercase">Observation Forecast</h1>
          <p className="text-muted-foreground text-lg">Predictive imaging quality for your locations.</p>
        </div>
        {isLoading && (
          <div className="flex items-center gap-2 px-4 py-2 bg-primary/10 rounded-full border border-primary/20 animate-pulse">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-primary">Syncing weather algorithms...</span>
          </div>
        )}
      </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {config?.locations.map(loc => {
              const isDefault = loc.default || loc.name === activeLocation?.name;
              const isSelected = selectedLocationNames.includes(loc.name) || isDefault;
              return (
                <button
                  key={loc.name}
      ...
      {error && !hasAnyData && (
          <div className="p-12 text-center bg-rose-500/5 border border-rose-500/20 rounded-3xl">
              <h3 className="text-xl font-black text-rose-500 uppercase tracking-tighter mb-2">Sync Error</h3>
            ...
            <div className="flex flex-wrap gap-4 px-2">
                {chartData.datasets.map((ds, i) => (
                    <div key={i} className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: ds.borderColor }} />
                        <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">{ds.label}</span>
                    </div>
                ))}
            </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6">
            {daysGrid.map((day, i) => {
                    ...
                    return (
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h4 className="text-lg font-black">{date.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</h4>
                                </div>
                            </div>
                ...
                const forecastDay = day as { displayLocations: any[], bestLocationName: string | null, date: string };

                return (
                    <div
                        key={i}
                        onClick={() => setSelectedDay(day)}
                        className={`p-4 border rounded-3xl bg-card shadow-sm flex flex-col transition-all hover:border-primary/40 cursor-pointer hover:shadow-md hover:scale-[1.02] active:scale-[0.98] ${isToday ? 'ring-2 ring-primary ring-offset-4 ring-offset-background' : ''}`}
                    >
                        <div className="flex justify-between items-start mb-4 min-w-0">
                            <div className="min-w-0">
                                ...
                                <h4 className="text-lg font-black truncate">{date.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</h4>
                            </div>
                            {isToday && (
                                <span className="bg-primary text-primary-foreground text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-widest shrink-0">Today</span>
                            )}
                        </div>
                            ...
                            {forecastDay.displayLocations.map((loc, j) => {
                                ...
                                return (
                                    <div key={j} className="group/item flex-1 flex flex-col">
                                        <div className={`flex flex-col h-full p-2.5 rounded-xl border transition-all ${
                                          isBest
                                            ? 'bg-primary/5 border-primary/20 ring-1 ring-primary/10'
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between mb-1.5 min-w-0">
                                                    <div className="flex flex-col min-w-0 pr-1">
                                                        <div className="flex items-center gap-1 mb-0.5">
                                                            <MapPin className={`w-2.5 h-2.5 shrink-0 ${isBest ? 'text-primary' : 'text-muted-foreground'}`} />
                                                            <span className={`text-[10px] font-bold truncate leading-none ${isBest ? 'text-primary' : ''}`}>
                                                              {loc.name}
                                                            </span>
                                                        </div>
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            <span className="text-[9px] font-medium text-muted-foreground leading-none">
                                                                {loc.effective_hours ? `${loc.effective_hours.toFixed(1)}h` : '0h'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <div className="flex gap-1.5 shrink-0">
                                                        <div className="flex flex-col items-center">
                                                            <span className="text-[7px] font-black text-muted-foreground uppercase tracking-tighter leading-none mb-0.5">Abs</span>
                                                            <QualityBadge score={loc.absolute_quality} />
                                                        </div>
                                                        <div className="flex flex-col items-center">
                                                            <span className="text-[7px] font-black text-muted-foreground uppercase tracking-tighter leading-none mb-0.5">Rel</span>
                                                            <QualityBadge score={loc.relative_quality} />
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                {isBest && (
                                                  <div className="mb-1.5 px-1.5 py-0.5 bg-primary/10 rounded-md self-start">
                                                    <span className="text-[8px] font-black uppercase text-primary tracking-tighter">Best Site</span>
                                                  </div>
                                                )}

                                                {loc.note && (
                                                    <div className={`flex items-start gap-1 mb-2 p-1.5 rounded-lg ${isBest ? 'bg-primary/5' : 'bg-background/50'}`}>
                                                        <AlertCircle className="w-2.5 h-2.5 text-amber-500 mt-0.5 shrink-0" />
                                                        <p className="text-[9px] text-muted-foreground leading-tight italic line-clamp-2">{loc.note}</p>
                                                    </div>
                                                )}
                                            </div>
