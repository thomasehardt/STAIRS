...
import { Cloud, Wind, Thermometer, Moon, MapPin, Info, Sparkles, Activity, Plus, Minus, Filter, X, Eye } from 'lucide-react';
import { formatTemp } from '@/lib/utils';

export function Dashboard() {
    const { activeLocation, tempUnit } = useSettings();
    const { data: recommendations, isLoading: targetsLoading } = useRecommendedTargets();
    ...
    const isDarkNow = nightStart && nightEnd && now >= nightStart && now <= nightEnd;
    ...
    useEffect(() => {
        if (recommendations && recommendations.length > 0 && visibleTargetIds.length === 0 && scoreFilter === null && categoryFilter.length === 0) {
            setVisibleTargetIds(recommendations.slice(0, 5).map(r => r.target_id));
        }
    }, [recommendations, visibleTargetIds.length, scoreFilter, categoryFilter.length]);
    ...
    useEffect(() => {
        ...
        const filteredIds = recommendations
            .filter(r => {
                const matchesScore = scoreFilter === null || r.final_score >= scoreFilter;
                const matchesCategory = categoryFilter.length === 0 || categoryFilter.includes(r.target_type.trim());
    ...
    const allTargetIds = useMemo(() => (recommendations || []).map(r => r.target_id), [recommendations]);

    // Data for Sky Visualization (Fetch ALL once for instant toggling)
    const { data: skyViewData, isLoading: skyViewLoading } = useSkyView(allTargetIds, nightStart?.toISOString() || null);
    ...
    const currentQuality = useMemo(() => {
        ...
        const closest = points.reduce((prev, curr) =>
            Math.abs(new Date(curr.time).getTime() - nowMs) < Math.abs(new Date(prev.time).getTime() - nowMs) ? curr : prev
        );

        return {
            rel: Math.round(closest.score),
            abs: Math.round(closest.score * (activeLocation?.bortle_scale ? (1 - (activeLocation.bortle_scale - 1) / 8) : 1))
        };
    ...
    return (
        <div className="space-y-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                    <h1 className="text-4xl font-black tracking-tight uppercase">Sky Dashboard</h1>
                ...
                {nightStart && nightEnd && (
                    <div className="bg-primary/10 border border-primary/20 px-4 py-2 rounded-2xl flex items-center gap-3 group relative cursor-help">
                        <div className={`w-2 h-2 rounded-full ${isDarkNow ? 'bg-primary animate-pulse' : 'bg-muted-foreground'}`} />
                        <div>
                            <p className="text-[10px] font-bold text-primary uppercase tracking-wider">{timeLabel}</p>
                            <p className="text-sm font-mono font-bold">
                                {nightStart.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} -
                                {nightEnd.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </p>
                        </div>
                        ...
                        <div className="absolute top-full right-0 mt-3 w-64 p-3 bg-popover text-[10px] rounded-xl border shadow-2xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-50 transform -translate-y-2 group-hover:translate-y-0">
                            <p className="font-bold text-primary mb-1 uppercase tracking-widest">Astronomical Night</p>
                            <p className="text-muted-foreground leading-relaxed">
                                This is the period when the sun is more than 18° below the horizon.
                            </p>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <WeatherWidget label="Clouds" value={weather?.cloud_cover_pct !== null ? `${weather?.cloud_cover_pct}%` : 'N/A'} icon={Cloud} loading={weatherLoading} />
                    <WeatherWidget label="Humidity" value={weather?.humidity_pct !== null ? `${weather?.humidity_pct}%` : 'N/A'} icon={Wind} loading={weatherLoading} />
                    <WeatherWidget label="Temp" value={formatTemp(weather?.temperature_c, tempUnit)} icon={Thermometer} loading={weatherLoading} />
                    <WeatherWidget label="Seeing" value={weather?.seeing !== null ? `${weather?.seeing}"` : 'Good'} icon={Moon} loading={weatherLoading} />
                    <div className="p-4 bg-primary/5 border border-primary/20 rounded-2xl flex items-center gap-4 hover:border-primary/40 transition-colors group">
                        ...
                        <div className="min-w-0">
                            <p className="text-[10px] font-bold text-muted-foreground uppercase leading-none mb-1">Sky Score</p>
                            {qualityLoading ? (
                                <div className="h-5 w-16 bg-secondary animate-pulse rounded" />
                            ) : currentQuality ? (
                                <div className="flex items-baseline gap-2">
                                    <span className="text-lg font-black text-primary">{currentQuality.rel}%<span className="text-[8px] ml-0.5 opacity-50 uppercase font-bold">Rel</span></span>
                                    <span className="text-lg font-black text-primary/70">{currentQuality.abs}<span className="text-[8px] ml-0.5 opacity-50 uppercase font-bold">Abs</span></span>
                                </div>
                            ) : (
                        ...
                        <div className="flex items-center justify-between px-1">
                            <div className="flex bg-secondary/50 p-1 rounded-xl border border-border/50">
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${dashboardView === 'paths' ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${dashboardView === 'sky' ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
                                >
                            ...
                            <div className="p-1 hover:bg-secondary rounded-full cursor-help group relative">
                                ...
                                <div className="absolute bottom-full right-0 mb-2 w-56 p-3 bg-popover text-[10px] rounded-xl border shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                                    <p className="font-bold text-primary mb-1 uppercase">Dynamic Visualization</p>
                            ...
                            {dashboardView === 'paths' ? (
                                posLoading || qualityLoading ? (
                                    ...
                                    <div className="flex-1 min-h-0">
                                        <MultiAltitudeChart targetDatasets={combinedPositions} qualityPoints={points} />
                                    </div>
                                ) : (
                                    <div className="h-full flex items-center justify-center text-muted-foreground italic text-xs text-center p-8 flex-1">
                                        No paths selected. Toggle targets from the list to see their orbit.
                                    </div>
                                )
                            ) : (
                                skyViewLoading ? (
                    ...
                    <div className="p-6 border rounded-[2.5rem] bg-card space-y-6 shadow-sm">
                        <div className="flex items-center justify-between">
                            ...
                            {(scoreFilter !== null || categoryFilter.length > 0) && (
                                <button onClick={handleClearFilters} className="text-[10px] font-bold text-muted-foreground hover:text-foreground flex items-center gap-1">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">Min Score</p>
                                <div className="flex gap-1">
                                    {[60, 70, 80, 90].map(s => {
                                        const hasTargets = recommendations?.some(r => r.final_score >= s);
                                        return (
                                            <button
                                                key={s}
                                                disabled={!hasTargets}
                            ...
                            <div className="space-y-2">
                                <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">Categories</p>
                                <div className="flex flex-wrap gap-1.5">
                                    {categories.map(cat => {
                                        const hasTargets = recommendations?.some(r => r.target_type.trim() === cat);
                                        return (
                                            <button
                                                key={cat}
                                                disabled={!hasTargets}
                        ...
                        <div className="pt-4 border-t border-border/50">
                            <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest mb-3">Individual Toggles</p>
                            <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                {recommendations?.map(r => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {isVisible ? <Minus className="w-2.5 h-2.5" /> : <Plus className="w-2.5 h-2.5" />}
                                            <span className="truncate max-w-[80px]">{r.target_id}</span>
                                            <div className="flex items-center gap-1 ml-1 pl-1 border-l border-current/20">
                                                <span>{Math.round(r.sqs_score)}<span className="text-[7px] opacity-60 ml-0.5">R</span></span>
                                                <span className="opacity-30">|</span>
                                                <span>{Math.round(r.aqs_score || r.final_score)}<span className="text-[7px] opacity-60 ml-0.5">A</span></span>
                                            </div>
                                        </button>
                                    );
                                })}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {targetsLoading ? (
                            Array.from({ length: 6 }).map((_, i) => (
                                <div key={i} className="h-[400px] bg-secondary/20 animate-pulse rounded-[2rem]" />
                            ))
