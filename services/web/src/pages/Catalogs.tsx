...
import { Search, Loader2, Filter, Eraser, ChevronDown, ChevronUp } from 'lucide-react';
...
export function CatalogsPage() {
  ...
  const { data: targets, isLoading, isFetching } = useTargets({
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight uppercase">Deep Sky Catalogs</h1>
          <p className="text-muted-foreground text-lg">Discovery and exploration of the visible universe.</p>
        </div>
        <div className="flex items-center gap-2">
            {isFetching && (
                <div className="flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full border border-primary/20">
                    <Loader2 className="w-3 h-3 animate-spin text-primary" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-primary">Searching...</span>
                </div>
            )}
            <Button
                variant="outline"
                ...
                className={`rounded-xl gap-2 ${showFilters ? 'bg-primary/10 border-primary/40' : ''}`}
            >
        ...
        <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex bg-secondary/50 p-1 rounded-2xl border border-border/50">
                {CATALOGS.map((cat) => (
                    <button
                        key={cat}
                        onClick={() => { setCatalog(cat); setSearch(''); }}
        ...
        {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 p-8 bg-card border border-border rounded-[2.5rem] shadow-sm animate-in slide-in-from-top-4 duration-300">
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Object Type</label>
                    <select
                        value={targetType}
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Constellation</label>
                    <select
                        value={constellation}
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary flex justify-between">
                        ...
                        <span className="font-mono text-foreground">{maxMagnitude}</span>
                    </label>
                    <input
                        type="range"
                        min="0"
      ...
      {isLoading ? (
        <div className="h-96 flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-muted-foreground font-black uppercase tracking-widest text-xs">Accessing Star Maps...</p>
        </div>
      ) : (
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                {targets?.results.map((target) => (
                    <TargetCard
                        ...
                        isPinned={pinnedTargets.some(pt => pt.id === target.identifier)}
            ...
            {targets?.results.length === 0 ? (
                <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-[2.5rem] bg-card text-muted-foreground">
                    <p className="font-black uppercase tracking-widest">No cosmic matches found</p>
                    <p className="text-sm mt-2 opacity-60 italic">Try adjusting your filters or search query.</p>
                </div>
                ...
                <div className="text-center pt-8 border-t border-border/50">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">End of catalog results</p>
                </div>
            )}
        </div>
      )}
