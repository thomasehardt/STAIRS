...
import { Telescope, MapPin, Activity, Ruler, Info, Loader2, Pin, PinOff, Calendar, Plus, Star, Trash2 } from 'lucide-react';
...
export function TargetDetailPage() {
  ...
  const { data: target, isLoading: targetLoading } = useQuery({
    queryKey: ['target', id],
    queryFn: async () => {
      const { data } = await api.get<TargetDetail>(`/targets/${id}?profile_name=${activeTelescope}`);
      return data;
    },
    enabled: !!id,
  });

  const { data: posData, isLoading: posLoading } = useTargetPosition(id || '', nightStart, nightEnd);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-muted-foreground font-mono text-sm">{target.catalog_id}</span>
          </div>
          <h1 className="text-5xl font-black tracking-tight">{target.identifier}</h1>
          <p className="text-2xl text-muted-foreground font-medium">{target.common_name || 'Deep Sky Object'}</p>
        </div>
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="p-8 border rounded-[2.5rem] bg-card space-y-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <DetailItem label="Constellation" value={target.constellation || 'N/A'} icon={MapPin} />
              <DetailItem label="Magnitude" value={target.magnitude?.toString() || 'N/A'} icon={Activity} />
              <DetailItem label="Size" value={target.angular_size ? `${target.angular_size[0]}' × ${target.angular_size[1]}'` : 'N/A'} icon={Ruler} />
              <DetailItem label="Distance" value={target.distance ? `${target.distance} kly` : 'N/A'} icon={Telescope} />
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-1 space-y-8">
          ...
          <div className="p-8 border rounded-[2.5rem] bg-card space-y-6 shadow-sm flex flex-col">
            <div className="flex items-center justify-between">
                ...
                >
                    {isAddingLog ? <PinOff className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                </Button>
            </div>

            {isAddingLog && (
                <div className="p-4 bg-secondary/20 rounded-2xl border border-border space-y-4">
                    <h4 className="text-xs font-black uppercase tracking-widest text-muted-foreground">Log New Session</h4>
                        ...
                        <div className="flex gap-2">
                            {[1, 2, 3, 4, 5].map(star => (
                                <button
                                    ...
                                    className={`p-1 transition-colors ${newLogRating >= star ? 'text-amber-400' : 'text-muted-foreground/30 hover:text-amber-400/50'}`}
                                >
                    ...
                    <div className="flex gap-2 pt-2">
                        ...
                        >
                            {createLogMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Save Log'}
                        </Button>
                        <Button
                            variant="outline"
            ...
            <div className="flex-1 space-y-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
               {logsLoading ? (
                 ...
                 logs.map(log => (
                     <div key={log.id} className="p-4 bg-background/50 rounded-2xl border border-border/50 space-y-2 group relative">
                         <div className="flex items-center justify-between">
                            <span className="text-xs font-mono font-bold text-muted-foreground">{new Date(log.session_date).toLocaleDateString()}</span>
                            <div className="flex gap-0.5">
                                {[...Array(5)].map((_, i) => (
                                    <Star key={i} className={`w-3 h-3 ${i < (log.rating ?? 0) ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/30'}`} />
                                ))}
                            </div>
                         </div>
                         {log.notes && (
                             <p className="text-sm italic text-foreground/80">{log.notes}</p>
                         )}
                 ...
                 <div className="text-center py-8 space-y-2">
                     ...
                     <p className="text-sm font-bold text-foreground">No logs yet.</p>
                     <p className="text-xs text-muted-foreground px-4">Record your first observation of this target.</p>
                 </div>
               )}
            </div>
          </div>
